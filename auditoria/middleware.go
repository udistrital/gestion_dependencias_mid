...
func ListenRequest(ctx *context.Context) {
	...
	fmt.Println("Antes de asignar variables");
	...
	go func() {
		defer func() {
			...
			if r := recover(); r != nil {
				...
				fmt.Println("Entra en primer if");
				if end_point != "/" {
					fmt.Println("entra en if de endpoint");
					beego.Info(log)
				}
		...
		defer func() {
			if r := recover(); r != nil {
				fmt.Println("Entra en segundo defer");
				// access_token = "Error WSO2"
				user = "Error wso2"
				var log = fmt.Sprintf(`@&%s@&%s@&%s@&%s@&%s@&%s@&%s@&%s@&%s@$`, app_name, host, end_point, method, date, ip_user, user_agent, user, data_response)
				if end_point != "/" {
					fmt.Println("Entra en segundo if");
					beego.Info(log)
				}
			}
		}()
		...
		var log = fmt.Sprintf(`@&%s@&%s@&%s@&%s@&%s@&%s@&%s@&%s@&%s@$`, app_name, host, end_point, method, date, ip_user, user_agent, user, data_response)
		if end_point != "/" {
			fmt.Println("Entra en tercer if");
			beego.Info(log)
		}
	}()
}
